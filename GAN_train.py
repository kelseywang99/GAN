...
def train_gan(G, D, opt_D, opt_G, criterion, save_dir, dataloader, latent_dim, n_epoch):
    ...
    # 存储参数
    torch.save(G.state_dict(), os.path.join(save_dir, f'gan_g.param'))
    torch.save(D.state_dict(), os.path.join(save_dir, f'gan_d.param'))
...
if __name__ == '__main__':
    ...
    # faces文件夹地址
    save_dir =  sys.argv[2]
    # g.param、d.param要保存在save_dir文件夹下，例如save_dir = path/log,则参数保存为 path/log/gan_g.param, path/log/gan_d.param
    n_epoch =  sys.argv[3]
